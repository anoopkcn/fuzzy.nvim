...
local function run(dir, raw_args)
    local dedupe_lines = require("fuzzy.config").get().grep_dedupe
    local label = dedupe_lines and "FuzzyGrepIn" or "FuzzyGrepIn!"
    local title = label .. " [" .. vim.fn.fnamemodify(dir, ":~") .. "]"
            ...
            if not batch_scheduled then
                ...
                vim.schedule(function()
                    ...
                    for _, raw_line in ipairs(batch) do
                        ...
                        if e then
                            e.filename = util.with_root(e.filename, dir)
                            if seen then
